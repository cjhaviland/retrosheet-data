...
function getData(file, yearToProcess) {
    ...
    // const teamJson = JSON.parse(fs.readFileSync(`./data/${yearToProcess}/json/${file}`))
    ...
    for (let item of teamObj) {
        if (item) {
            ...
            createEntry(item.resBatter, item.gameId, true, 'New Batter')
            createEntry(item.resPitcher, item.gameId, false, 'New Pitcher')
    ...
    fs.writeFileSync(`./data/output/${yearToProcess}/batterOutput.json`, JSON.stringify(batterResult, null, 2))
    fs.writeFileSync(`./data/output/${yearToProcess}/pitcherOutput.json`, JSON.stringify(pitcherResult, null, 2))
}

function createEntry(player, gameId, isBatter, log) {
    if (isBatter) {
        if(!batterResult[`${player}`]) {
            // console.log(player, gameId, isBatter, log)
...
function batterStats(item) {
    ...
    if (item.sbForRunnerOn1stFlag === 'T') {
        createEntry(item.firstRunner, item.gameId, true, 'New SB on 1st')
    ...
    if (item.sbForRunneron2ndFlag === 'T') {
        createEntry(item.secondRunner, item.gameId, true, 'New SB on 2nd')
    ...
    if (item.sbForRunnerOn3rdFlag === 'T') {
        createEntry(item.thirdRunner, item.gameId, true, 'New SB on 3rd')
...
function calcRuns(dest, player, pitcherId, gameId) {
    if (dest >= 4) {
        createEntry(player, gameId, true, 'Calc Runs')
        ...
        if (dest === 4) {
            createEntry(pitcherId, gameId, false, 'Res Pitcher Calc Runs')
